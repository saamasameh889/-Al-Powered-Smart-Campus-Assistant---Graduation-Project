...
def _build_temporal(gpa_history: list[float], load_history: list[float]) -> np.ndarray:
    temporal = []
    for gpa, load in zip(gpa_history, load_history):
        gpa_norm  = gpa / 4.0
        load_norm = load / 24.0
        risk_flag = (
            1.0 if gpa < 2.0
...
def render_forecasting_page() -> None:
    ...
    st.markdown(
        ...
        "margin-bottom:4px'>📈 GPA Trajectory Forecasting</div>",
        unsafe_allow_html=True,
    )
    st.markdown(
        "<div style='font-size:.83rem;color:#4A3A7A;margin-bottom:20px'>"
        "LSTM with additive attention predicts your GPA for the next 3 semesters "
        "with calibrated 80% confidence intervals via quantile regression.</div>",
        unsafe_allow_html=True,
    )
    ...
    # Pre-fill from clustering result if available
    _cf = st.session_state.get("cluster_features", {})

    col_a, col_b = st.columns([1, 2])
    with col_a:
        programme      = st.selectbox("Programme", _PROGRAMMES, index=0)
        current_sem    = st.slider("Current Semester", 4, 8, 5)
        avg_attendance = st.slider(
            "Avg Attendance (%)", 50, 100,
            int(_cf.get("avg_attendance", 82)),
        )
        avg_final = st.slider(
            "Avg Final Score (%)", 30, 100,
            int(_cf.get("avg_final", 70)),
        )
        failed_ratio = st.slider(
            "Failed Course Ratio", 0.0, 1.0,
            float(_cf.get("failed_ratio", 0.05)), step=0.01,
        )
    ...
    if st.button("📈  Forecast My GPA", type="primary"):
        ...
        temporal_x = _build_temporal(gpa_history, load_history)
    ...
    sem_start = (
        st.session_state.get("_last_start_sem", 1) + HISTORY_LEN
    )
    ...
    with st.expander("🎛️  What-If Scenario Analysis", expanded=True):
        ...
        wi_attendance = wc1.slider(
            ...
            int(avg_attendance), key="wi_att",
        )
        wi_load = wc2.slider(
            "New credit load", 9, 24, 18, key="wi_load",
        )
        run_wi = st.button("▶  Compute What-If", key="wi_btn")
